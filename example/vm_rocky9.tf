resource "vsphere_virtual_machine" "example" {
  ...
  disk {
    label        = "disk0"
    attach       = true
    path         = vsphere_file.rocky_cloudinit.destination_file
    datastore_id = data.vsphere_datastore.this.id
  }
  ...
  lifecycle {
    replace_triggered_by = [ vsphere_file.rocky_cloudinit ]
  }
}

resource "vsphere_file" "rocky_cloudinit" {
  ...
  source_file        = "base/rocky9/disk.vmdk"
